...
export default function AddPaletteModal({ navigation }) {
  const [name, setName] = useState('');
  const [selectedColours, setSelectedColours] = useState([]);

  const handleSubmit = () => {
    if (!name || selectedColours.length < 3) {
      Alert.alert('Please enter a palette name and select at least 3 colours');
    } else {
      ...
      console.log('newColourPalette', newColourPalette);
    }
  };

  const handleColourChange = (value, colour) => {
    // console.log('handleColourChange', value, colour);
    if (value === true) {
      setSelectedColours((current) => [colour, ...current]);
    } else {
      setSelectedColours((current) =>
        current.filter((c) => c.colorName !== colour.colorName),
      );
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.subHeading}>Name of your colour scheme</Text>
      <TextInput
            ...
            <Switch
              value={
                !!selectedColours.find(
                  (colour) => colour.colorName === item.colorName,
                )
              }
              onValueChange={(selected) => handleColourChange(selected, item)}
            />
          </View>
        )}
      />
